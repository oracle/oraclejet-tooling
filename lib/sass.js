...
function _getSassTaskPromise(options, context) {
  let sass;
  const pathToNodeSass = path.join(path.resolve('node_modules'), 'node-sass');
  // check if node-sass module is installed
  if (fs.existsSync(pathToNodeSass)) {
    sass = require(pathToNodeSass); // eslint-disable-line
  } else {
    util.log.error('node-sass is not installed. To install it, run: ojet add sass.');
  }
