...
function _getSassDest(buildType, dest, min) {
  ...
  if (isCcaSassFile) {
    ...
    if (components.length === 1) {
      const dir = min ? '../min' : '..';
      sassDest = util.destPath(path.join(base, topDir, components[0], dir, name + ext));
    } else {
...
function _getSassTasksFromFileList(theme, fileList, context, isCcaLiveReload) {
  ...
  fileList.forEach((file) => {
    const src = file.src;
    const dest = _getSassDest(buildType, file.dest, false);
    if (!src || path.basename(src)[0] === '_') {
      ...
      if (buildType === 'release' && util.isCcaSassFile(file.dest)) {
        ...
        const fileOpts2 = {
          file: src,
          outFile: _getSassDest(buildType, file.dest, true)
        };
