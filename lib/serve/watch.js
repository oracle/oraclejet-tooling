  ...
  return new Promise((resolve, reject) => {
    try {
      Object.keys(opts).forEach((watchTarget) => {
        ...
        watchers[watchTarget] = _startWatcher(opts[watchTarget].files, watchTarget, intervalValue, opts);
      });
      resolve();
    } catch (err) {
      reject(err);
...
function _startWatcher(targetFiles, target, intervalValue, opts) {
  const watcher = new Gaze(targetFiles, { interval: intervalValue });
