...
module.exports = function (filePath, buildContext) {
  return new Promise((resolve) => {
    ...
    if (_isIndexHtml(filePath)) {
      ...
      let version = util.getFromComponentVersionCache({ component });
      // If the simple name lookup fails, then we check to see if we have a jetpack component.
      if (version === undefined) {
        component = util.getJetpackCompNameFromConfigObj(pathComponents.end);
        if (component !== null) {
          version = util.getFromComponentVersionCache({ component });
        }
      }
