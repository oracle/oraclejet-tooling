...
function _getPackWebDirPath(options, configPaths, packName, packVersion) {
  const omitComponentVersion = options && options[CONSTANTS.OMIT_COMPONENT_VERSION_FLAG];
  // Ensures that setting versionless structure flag in the oraclejet config file enables
  // a versionless structure in the staging folder. And that the omit component version
  // flag still operates in projects that have the versionless structure not configured.
  const shouldBeVersionless = omitComponentVersion ||
  ...
  if (!packWebDirPath) {
    packWebDirPath = shouldBeVersionless ? unVersionedPackWebDirPath : versionedPackWebDirPath;
  }
