...
pack.package = function (packNames, options) {
  return new Promise((resolve) => {
    ...
    // We allow publishing of only a single pack
    // User can script a loop if he want to package multiple packs
    ...
    if (existsInWebDir || existsInHybridDir) {
      // Save available components names for packaging
      ...
      Object.keys(packComponentJson.dependencies).forEach((packComponentName) => {
        ...
        const packComponentComponentJsonPath = path.join(
          CONSTANTS.APP_SRC_DIRECTORY,
          sourceScriptsFolder,
          CONSTANTS.JET_COMPOSITE_DIRECTORY,
          packName,
      ...
      const packagePackComponents = function () {
        return new Promise((res, rej) => {
          let i = 0;
          function fn() { // eslint-disable-line
            if (i < packComponentNames.length) {
              component.package([packComponentNames[i]], Object.assign(opts, {
                pack: packName,
                _output: path.join(CONSTANTS.PACKAGE_OUTPUT_DIRECTORY, `${packComponentNames[i]}.zip`),
                _suppressMsgColor: true
              }))
                .then(() => {
                  i += 1;
      ...
      // If packing only a pack excluding its dependencies, just return resolved Promise
      const initPackagePromise = opts._excludeDependencies ?
        () => { return Promise.resolve(); } // eslint-disable-line
        : packagePackComponents;
      ...
      util.loginIfCredentialsProvided()
        ...
        .then(() => { // eslint-disable-line
          ...
          return opts._excludeDependencies ?
            () => { return Promise.resolve(); } // eslint-disable-line
            : _verifyPackComponents(packName, packComponentNamesAndVersions);
        })
