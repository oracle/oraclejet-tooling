...
function _shufflePackComponentResources(componentMetadata) {
  return new Promise((resolve) => {
    const { pack, name } = componentMetadata;
    if (pack) {
      const pathToPack = path.join(componentsDirPath, pack);
      const pathToPackComponent = path.join(pathToPack, name);
      const pathToPackComponentMinFolder = path.join(pathToPackComponent, 'min');
      const pathToPackComponentTypesFolder = path.join(pathToPackComponent, 'types');
      if (util.fsExistsSync(pathToPackComponentMinFolder)) {
        fs.moveSync(pathToPackComponentMinFolder, path.join(pathToPack, 'min', name));
      }
      if (util.fsExistsSync(pathToPackComponentTypesFolder) &&
        util.isDirectory(pathToPackComponentTypesFolder)) {
        // only copy *.ts files in case a component uses the "types" folder
        // for none-typescript resources
        glob.sync('**/*.ts', { cwd: pathToPackComponentTypesFolder }).forEach((tsFile) => {
          fs.moveSync(
            path.join(pathToPackComponentTypesFolder, tsFile),
            path.join(pathToPack, 'types', name, tsFile)
          );
        });
        // if folder only had *.ts files, delete it because all the relevant files were moved
        if (!fs.readdirSync(pathToPackComponentTypesFolder).length) {
          fs.removeSync(pathToPackComponentTypesFolder);
        }
      }
    }
    resolve(componentMetadata);
