  ...
  return new Promise((resolve) => {
    ...
    if (!opts.pack) {
      ...
      if (fs.existsSync(componentComponentJsonPath)) {
        ...
        util.log.error(`Component's main file '${componentComponentJsonPath}' does not exist.`);
      }
    } else {
      // Pack component
      ...
      if (fs.existsSync(packComponentJsonPath)) {
        ...
        if (fs.existsSync(componentComponentJsonPath)) {
          ...
          util.log.error(`Pack component's main file '${componentComponentJsonPath}' does not exist.`);
        }
      } else {
        util.log.error(`Pack's main file '${packComponentJsonPath}' does not exist.`);
      }
    }
    ...
    if (existsInWebDir || existsInHybridDir) {
      ...
      if (opts._accessToken) {
        initialPromise = new Promise((res) => {
          res(opts._accessToken);
        });
      } else {
        initialPromise = (user && typeof user !== 'boolean' && pass && typeof pass !== 'boolean') ?
