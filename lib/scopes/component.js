...
component.package = function (componentNames, options) {
  return new Promise((resolve) => {
    ...
    // User can script a loop if he want to package multiple component
    const componentName = componentNames[0];
    const componentPath = _getComponentPath(componentName, opts);
    ...
    // of pack's components is more complex (see _getComponentPath()) we exclude this case
    if (!opts.pack) {
      // Read the metadata of a first level component
      const componentComponentJsonPath = path.join(
...
function _getComponentPath(componentName, opts) {
  const sourceScriptsFolder = util.getSourceScriptsFolder();
...
component.publish = function (componentName, options) {
  return new Promise((resolve) => {
    ...
    let componentPath = '';
    if (!opts.path) {
      componentPath = _getComponentPath(componentName, opts);
    }
