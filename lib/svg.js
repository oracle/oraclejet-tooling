...
function _svgSprite(context) {
  ...
  if (!util.getInstalledCssPackage()) {
    themePath = path.join(`${config('paths').staging.themes}`, CONSTANTS.DEFAULT_THEME);
  } else {
    themePath = path.join(`${config('paths').staging.themes}`, CONSTANTS.DEFAULT_PCSS_THEME);
  }
  return new Promise((resolve, reject) => {
    try {
  ...
  return {
    css: {
      render: {
        scss: {
          template: `${CONSTANTS.PATH_TO_ORACLEJET}/scss/templates/svg-sprite-template.scss.txt`,
          dest: `${CONSTANTS.PATH_TO_ORACLEJET}/pcss/oj/v${util.getJETVersion()}/oj/icons/themes/redwood/common/_oj-sprite.scss`
        }
      },
      layout: 'horizontal',
      dest: '.',
