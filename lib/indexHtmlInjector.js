...
function _getStyleLinkBase(css, theme, buildType) {
  ...
  if (masterJson !== null) {
    ...
    const cdnObj = { created: `${masterJson.cdns.jet.css}/${theme.name}${platform}/oj-${theme.name}${linkExt}` };
...
function _getThemeStyleLink(theme, buildType) {
  ...
  if (linkBase.default) {
    ...
    themeStyleLink = theme.name === 'redwood' || theme.name === 'stable' ? `${linkToDefaultTheme}\n${linkToCreatedTheme}\n${linkToPreact}` : `${linkToDefaultTheme}\n${linkToCreatedTheme}`;
    return themeStyleLink;
  }
  linkToCreatedTheme = `<link rel="stylesheet" href="${linkBase.created}" id="basestyles" />`;
  themeStyleLink = theme.name === 'redwood' || theme.name === 'stable' ? `${linkToCreatedTheme}\n${linkToPreact}` : `${linkToCreatedTheme}`;
  return themeStyleLink;
}
