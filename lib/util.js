...
util.getInstallerCommand = ({ options }) => {
  ...
  return installerCmd === 'yarn' ? { installer: 'yarn', verbs: { install: 'add' } } : { installer: 'npm', verbs: { install: 'install' } };
};
...
util.getLibVersionsObj = () => {
  const versionsObj = {
    ...
    'ojs/ojcss': _getVersionFromNpm('oraclejet'),
...
util.getToolingPath = () => {
  ...
  source = path.dirname(require.resolve(`${CONSTANTS.ORACLEJET_TOOLING_NAME}/package.json`));
  if (util.fsExistsSync(source)) {
    return source;
  }
  // Not found anywhere
...
util.getOraclejetPath = () => {
  let source = path.resolve(`${CONSTANTS.ORACLEJET_PATH}`);

  if (util.fsExistsSync(source)) {
    return source;
  }
  source = path.dirname(require.resolve(`${CONSTANTS.ORACLEJET_NAME}/package.json`));
  if (util.fsExistsSync(source)) {
    return source;
  }
  // Not found anywhere
