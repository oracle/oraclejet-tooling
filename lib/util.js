...
util.getInstalledCssPackage = () => {
  let getPackageJson = path.resolve('./node_modules/postcss-custom-properties');
...
util.getCCANameFromPath = function (filePath) {
  ...
  const scriptsFolder = normalizedFilePath.split(path.sep).indexOf('ts') !== -1 ? 'ts' : 'js';
  const rootDir = path.join(scriptsFolder, CONSTANTS.JET_COMPOSITE_DIRECTORY);
...
/**
 * ## getComponentVersionCache
 *
 * @private
 * @return {Object} component version cache
 *
 */
util.getComponentVersionCache = () => (config('componentVersionObj') || {});

/**
 * ## getFromComponentVersionCache
 *
 * @private
 ...
 *
 */
util.getFromComponentVersionCache = ({ component }) => {
  const cache = util.getComponentVersionCache();
  return cache[component];
};

/**
 * ## addToComponentVersionCache
 *
 * @private
 * @param {string} options.component
 * @param {string} options.version
 */
util.addToComponentVersionCache = ({ component, version }) => {
  const cache = util.getComponentVersionCache();
  config('componentVersionObj', {
    ...cache,
    [component]: version
  });
};
...
util.getComponentVersion = function ({ pack = '', component }) {
  ...
  const cachedComponentVersion = util.getFromComponentVersionCache({
    component: fullComponentName
  });
  if (cachedComponentVersion) {
    return cachedComponentVersion;
  }
  let version;
  ...
  util.addToComponentVersionCache({ component: fullComponentName, version });
  return version;
};
...
util.getJetpackCompNameFromConfigObj = function (pathComponentsEnd) {
  const componentVersionCache = util.getComponentVersionCache();
  const matchingName = Object.keys(componentVersionCache).find(key => pathComponentsEnd.split(path.sep).indexOf(`${key.toString()}`) !== -1);
  return matchingName;
};
 ...
 *
 * @private
 * @param {string} pathComponents.start - start of component path
 * @param {string} pathComponents.mid - mid of component path
 * @param {string} pathComponents.end - end of component path
 * @returns {boolean}
 */
util.isTypescriptFile = function (pathComponents) {
  return pathComponents.end.endsWith('.ts') || pathComponents.end.endsWith('.tsx');
};
 ...
 * @private
 * @param {Object} options.context built context
 * @param {boolean} options.built whether to get component.json from
 * src or built folder
 * @returns {Array} array containing paths to
 * component.json files if any
 */
util.getLocalCompositeComponentJsonPaths = ({ context, built = false }) => {
  const configPaths = util.getConfiguredPaths();
  const componentJsonBasePath = util.pathJoin(
    configPaths.src.common,
    '*',
    configPaths.composites,
    '*',
  ...
  const componentJsonFilePaths = glob.sync(componentJsonBasePath);
  if (built) {
    return componentJsonFilePaths.map((filePath) => {
 ...
 * false otherwise
 */
util.shouldNotRunTypescriptTasks = context => !util.isTypescriptApplicaiton() ||
...
util.getSourceScriptsFolder = () => {
  const configPaths = util.getConfiguredPaths();
  return util.isTypescriptApplicaiton() ?
...
util.getComponentPathFromThemingFileDest = ({ dest, isReleaseBuild }) => {
  ...
  const javascriptBase = path.join(
    configPaths.staging.stagingPath,
    configPaths.src.javascript,
    configPaths.composites
  );
  ...
  // go from <staging>/(js|ts)/jet-composites/<component>/*/*.scss to
  // to <component>/* e.g. web/js/jet-composites/oj-foo/css/oj-foo-styles.scss to
  // oj-foo/css
  const topDir = path.normalize(dest).startsWith(javascriptBase) ?
    path.dirname(path.relative(javascriptBase, dest)) :
    path.dirname(path.relative(typescriptBase, dest));
  const [componentRoot, ...subFolders] = topDir.split(path.sep);
  let componentJson;
  if (subFolders.length && util.isWebComponent({ pack: componentRoot, component: subFolders[0] })) {
    // topDir corresponds to a pack component e.g oj-pack/oj-foo/oj-foo-styles.scss
    // results in componetRoot = oj-pack & subFolers[0] = oj-foo (pack component)
    componentJson = util.getComponentJson({
      pack: componentRoot,
      component: subFolders[0],
      built: true
    });
    // first subfolder corresponds to pack component name so remove it from the list since
    // we'll get it from the component.json
    subFolders.shift();
  } else {
    // topDir corresponds to a singleton component e.g oj-foo/oj-foo.style.scss results
    // in componentRoot = oj-foo
    componentJson = util.getComponentJson({ component: componentRoot, built: true });
  }
  const { name: componentName, pack: packName, version: componentVersion } = componentJson;
  const minDir = isReleaseBuild ? 'min' : '';
  let componentPath;
  if (packName) {
    const packVersion = util.getComponentVersion({ component: packName });
    componentPath = path.join(javascriptBase, packName, packVersion, minDir, componentName);
  } else {
    componentPath = path.join(javascriptBase, componentName, componentVersion, minDir);
  }
  return {
    componentPath,
 ...
 *
 * @private
 * @returns {Array} array with local vcomponent component.json
 * paths
 */
util.getLocalVComponentsComponentJsonPaths = ({ context }) => {
  const configPaths = util.getConfiguredPaths();
  return util.getLocalVComponents().map(component => (util.pathJoin(
    context.opts.stagingPath,
    configPaths.src.javascript,
    configPaths.composites,
    component,
...
util.getVComponentComponentJson = ({ context, pack = '', component, built = false }) => {
  ...
  const pathToBuiltVComponentBase = path.join(
    stagingPath,
    configPaths.src.javascript,
    configPaths.composites
  );
  let pathToBuiltVComponent;
  if (pack) {
    pathToBuiltVComponent = path.join(
      pathToBuiltVComponentBase,
      pack,
      util.getComponentVersion({ component: pack }),
      component,
      CONSTANTS.JET_COMPONENT_JSON
    );
  } else {
  ...
  return util.readJsonAndReturnObject(pathToBuiltVComponent);
};
...
util.addComponentToTsconfigPathMapping = ({ component, isLocal }) => {
  if (util.isTypescriptApplicaiton()) {
    const configPaths = util.getConfiguredPaths();
 ...
 */
util.pathJoin = (...paths) => (paths.join('/'));
