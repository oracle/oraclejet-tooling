 ...
 * one resolves
 * @private
 * @param {Array} promises array containing funcitons that
 * return promises
 * @returns {Promise} promise chain that runs
 * in sequence
 */
util.runPromisesInSequence = promises => (
  promises.reduce(
...
util.isTypescriptApplicaiton = () => (
  util.fsExistsSync(path.join('.', CONSTANTS.PATH_TO_TSCONFIG))
);
...
util.getJetPackPathMappingsForTypescript = () => {
  ...
  if (util.fsExistsSync(basePath)) {
    util.getDirectories(basePath).forEach((dir) => {
...
util.getSourceScriptsFolder = () => {
  ...
  return util.isTypescriptApplicaiton() ?
    paths.src.typescript : paths.src.javascript;
};
