...
util.processContextForHooks = function (context) {
  ...
  if (context) {
    ...
    } else if (context.userOptions) {
      obj.userOptions = context.userOptions;
    }
    ...
    } else if (context.requireJs) {
      obj.requireJs = context.requireJs;
    }
    if (context.opts && context.opts.requireJsEs5) {
      obj.requireJsEs5 = context.opts.requireJsEs5;
    } else if (context.requireJsEs5) {
      obj.requireJsEs5 = context.requireJsEs5;
    }
    if (context.opts) {
      obj.isRequireJsEs5 = context.opts.isRequireJsEs5;
    } else if (context.isRequireJsEs5) {
      obj.isRequireJsEs5 = context.isRequireJsEs5;
    }
    ...
    } else if (context.componentRequireJs) {
      obj.componentRequireJs = context.componentRequireJs;
    }
    if (context.watcher) {
      obj.watcher = context.watcher;
    }
    if (context.opts && context.opts.typescript) {
      obj.typescript = context.opts.typescript;
    } else if (context.typescript) {
      obj.typescript = context.typescript;
    }
  }
  // console.log('processContextForHooks obj ' + JSON.stringify(obj, null, ' '));
 ...
 */
util.isTypescriptComponent = function ({ component }) {
  // eslint-disable-next-line global-require
  const config = require('./config');
  // check if src/ts/jet-composites/component exists
...
util.getComponentJsonFilePaths = () => {
  ...
  return [
    ...glob.sync(`${javascriptBase}/*/${CONSTANTS.JET_COMPONENT_JSON}`),
    ...glob.sync(`${typescriptBase}/*/${CONSTANTS.JET_COMPONENT_JSON}`)
  ];
};
...
/**
 * ## getJetPackPathMappingForTypescript
 * @private
 * * @param {string} component name of jet pack
 * @returns {Object} jet pack path mapping
 * required for reliable referencing during
 * typescript compilation
 */
util.getJetPackPathMappingForTypescript = ({ component }) => {
  // eslint-disable-next-line global-require
  const config = require('./config');
  const pathMappings = {};
  ...
  if (util.fsExistsSync(componentPath)) {
    ...
    if (util.hasProperty(componentJson, 'type') &&
      componentJson.type === 'pack' &&
      util.hasProperty(componentJson, 'name') &&
...
/**
 * ## getJetPackPathMappingsForTypescript
 * @private
 * @returns {Object} jet pack path mappings
 * required for reliable referencing during
 * typescript compilation
 */
util.getJetPackPathMappingsForTypescript = () => {
  const basePath = util.getTypescriptComponentsSourcePath();
  let pathMappings = {};
  if (util.fsExistsSync(basePath)) {
    util.getDirectories(basePath).forEach((component) => {
      pathMappings = {
        ...pathMappings,
        ...util.getJetPackPathMappingForTypescript({ component })
      };
