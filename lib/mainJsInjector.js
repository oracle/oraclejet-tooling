...
//
// Update config.mainJs for ES5, replacing the path mappings that are delimited by
// the "injector" comment tags.
// (see defaultconfig.js for the default tags (look for "injectPaths")).
//
function _writeRequirePathToEs5File(context) {
  const config = context.opts.injectPaths;
  const buildType = context.buildType;
  const platform = context.platform;

  return new Promise((resolve, reject) => {
    const destDir = (buildType === 'release') ? config.destMainJsEs5 : config.mainJs;
...
module.exports = {
  injectPaths: function _injectpaths(context) {
    return new Promise((resolve, reject) => {
      try {
        ...
        newContext.opts.requireJs = pathGenerator.updateRJsOptimizerConfig(context, false);
  ...
  injectPathsEs5: function _injectpaths(context) {
    return new Promise((resolve, reject) => {
      try {
        const newContext = context;
        const pathsObj = pathGenerator.getPathsMapping(context, false, true);
        newContext.opts.requireJsEs5 = pathGenerator.updateRJsOptimizerConfig(context, true);
        _setMainPathMapping(newContext, pathsObj, true)
        .then(_writeRequirePathToEs5File)
        .then(() => {
          resolve(newContext);
        });
      } catch (error) {
