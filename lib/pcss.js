...
function _getPcssTaskPromise(options, context) {
  const sass = require(path.join(path.resolve('node_modules'), 'node-sass')); // eslint-disable-line
  const postcss = require(path.join(path.resolve('node_modules'), 'postcss')); // eslint-disable-line
  const pcssCalc = require(path.join(path.resolve('node_modules'), 'postcss-calc')); // eslint-disable-line
  const autoPrefix = require(path.join(path.resolve('node_modules'), 'autoprefixer')); // eslint-disable-line
  return new Promise((resolve, reject) => {
