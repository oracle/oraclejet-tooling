...
module.exports = {
  ...
  typescript: function _runTypescriptCompilation(context) {
    return new Promise((resolve) => {
      ...
      const localTypescriptPath = 'node_modules/typescript/bin/tsc';
      ...
      util.fsExists(CONSTANTS.PATH_TO_TSCONFIG, (fsExistsError) => {
        if (fsExistsError) {
          ...
          util.log('Compiling Typescript');
          fs.readJSON(CONSTANTS.PATH_TO_TSCONFIG).then((tsconfigJson) => {
            const pathToStaging = path.join('.', config('paths').src.common, context.opts.stagingPath);
            const pathToCommon = path.join('.', config('paths').src.common);
            // eslint-disable-next-line no-param-reassign
            tsconfigJson.compilerOptions.baseUrl = path.relative(pathToStaging, pathToCommon);
            // eslint-disable-next-line no-param-reassign
            tsconfigJson.include = ['./ts/**/*'];
            if (context.buildType === 'dev') {
              // eslint-disable-next-line no-param-reassign
              tsconfigJson.compilerOptions.outDir = path.join('.', config('paths').src.javascript);
            }
            fs.writeJSON(tsconfigStagingPath, tsconfigJson, { spaces: 2 }).then(() => {
              Promise.all([
                util.validGlobalTypescriptAvailable(),
                fs.pathExists(localTypescriptPath).then(pathExists => pathExists)
              ]).then((values) => {
                const [validGlobalTypescriptAvailable, localTypescriptAvailable] = values;
                ...
                childProcess.exec(compilerArguments.join(' '), (execError, stdout, stderr) => {
                  if (/No inputs were found/.test(stdout.toString())) {
                    util.log('\x1b[31mNo ts directory or .ts files found\x1b[0m');
                  }
                  if (execError) {
