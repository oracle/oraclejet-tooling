 ...
 * ## _isTypePack
 *
 * Returns true if the componentJson is a pack.
 *
 * @private
 * @param {Object} componentJson
 * @returns {boolean}
 */
function _isTypePack(componentJson) {
  if (util.hasProperty(componentJson, 'type') && componentJson.type === 'pack') return true;
  return false;
}
...
function _getReferenceDependencies(context, packName, componentJson) {
  ...
  Object.keys(componentJson.dependencies).forEach((el) => {
    ...
    if (el.indexOf(packName) === 0) {
      ...
      componentName = `${packName}/${el.substring(resourceDependencyIndex)}`;
    } else {
      // case for a singleton dependency.
      componentName = el;
    }
    const dependentComponentJson = _getComponentJson(context, componentName);
        ...
        if (util.hasProperty(componentJson, 'dependencies')) {
          ...
          Object.keys(componentJson.dependencies).forEach((el) => {
            if (el.startsWith(packName)) {
              ...
              if (!_isTypePack(subComponentJson)) {
                promises.push(
 ...
 */
function _getComponentJson(context, componentName) {
  const basePaths = _getComponentBasePaths({ context, component: componentName });
  const componentDirPath = `${basePaths.srcBase}/${componentName}/${CONSTANTS.JET_COMPONENT_JSON}`;
  return util.readJsonAndReturnObject(`${componentDirPath}`);
}
