...
function _copyFileOver(filePath, buildContext) {
  ...
  if (util.isPathCCA(path.join(pathComponents.mid, pathComponents.end))) {
    ...
    const name = pack || component;
    const version = util.getComponentVersion({ component: name });
    const basePathArray = pathComponents.end.split(path.sep);
    let basePath = '';

    const startIndex = name ? basePathArray.indexOf(name) + 1 :
      basePathArray.indexOf(configPaths.components) + 2;

    for (let i = startIndex; i < basePathArray.length; i++) {
      basePath = path.join(basePath, basePathArray[i]);
    }
    const ccaEndPath = path.join(config('paths').src.javascript, config('paths').components, name, version, basePath);
  ...
  if (length > 1 && splitted[length - 1] === 'index.html' &&
    ...
    indexHtmlInjector.injectHybridScriptTags(
      {
