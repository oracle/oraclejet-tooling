...
function _copyFileOver(filePath, buildContext) {
  ...
  if (util.isPathCCA(pathComponents.end)) {
    ...
    let version = util.getFromComponentVersionCache({ component: name });
    // If the simple name lookup fails, then we check to see if we have a jetpack component.
    if (version === undefined) {
      name = util.getJetpackCompNameFromConfigObj(pathComponents.end);
      if (name !== null) {
        version = util.getFromComponentVersionCache({ component: name });
      }
    }
