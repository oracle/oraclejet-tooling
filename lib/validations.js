...
module.exports = {
  ...
  platform: function _validatePlatform(platform) {
    ...
    if (fs.existsSync(config('paths').src.web)) {
...
function _validateSassInstall() {
  ...
  const sassPackageJson = util.getModulePath(path.join('node_modules', 'node-sass', 'package.json'), 'node-sass');

  try {
    fs.statSync(sassPackageJson);
  } catch (err) {
    util.log.error('Please run \'ojet add sass\' to configure your projects for SASS processing. \n OR \n Please run \'ojet add theming\' to configure your projects for PCSS processing.');
  }
}
