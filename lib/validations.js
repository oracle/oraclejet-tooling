...
module.exports = {
  ...
  getDefaultServeConfig: () => {
    return _getDefaultServeConfig();
  },

  getDefaultPlatformsConfig: () => {
    return _getDefaultPlatformsConfig();
  },
  
  getThemeObject: (themeStr, platform) => {
    return _getValidThemeObject(themeStr, platform);
  }
}

function _getDefaultBuildConfig(platform) {
  let result =  (platform === 'web') ? DEFAULT_CONFIG.build.web : DEFAULT_CONFIG.build.hybrid;
...
function _setValidThemeOption(options, platform) {
    let themeStr = _setDefaultTheme(options, platform);
...
function _getThemesArray(themesOption, platform) {
  if (themesOption[0] === CONSTANTS.RESERVED_Key_ALL_THEME) {
    ...
    let themesArray = [];
    let allThemes = util.getAllThemes();
    allThemes.forEach((singleTheme) => {
      let tempArray = _getThemesArrayForAllPlatforms(singleTheme);
      themesArray = themesArray.concat(tempArray);   
    });
    let altaThemes = CONSTANTS.SUPPORTED_PLATFORMS.reduce((previousValue, currentValue) => {
      return (currentValue === platform) && _checkThemeFileExist(CONSTANTS.DEFAULT_THEME, currentValue) ? 
    ...
    themesOption.forEach((themeStr) => {
      let themeName = _getThemeNameFromStr(themeStr);
      let themePlatform = _getThemePlatformFromStr(themeStr, platform);
    // Handle --themes=themeName:all situation 
      if (themePlatform === CONSTANTS.RESERVED_Key_ALL_THEME){
        let tempArray = _getThemesArrayForAllPlatforms(themeName);
        result = result.concat(tempArray);
      } else if (themeName === CONSTANTS.RESERVED_Key_ALL_THEME) {
        let tempArray = _getThemesArrayForAllThemes(themePlatform);
        result = result.concat(tempArray);
      } else {
...
function _getThemesArrayForAllPlatforms(themeName) {
  ...
  let tempArray = allPlatforms.reduce((previousValue, currentValue) => {
    return _checkThemeFileExist(themeName, currentValue) ? 
...
function _getThemesArrayForAllThemes(themePlatform) {
  ...
  let tempArray = allThemes.reduce((previousValue, currentValue) => {
    return _checkThemeFileExist(currentValue, themePlatform) ? 
...
function _processThemesOption(options, platform) {
  if (options.themes) {
    ...
    let defaultTheme = `${options.theme.name}:${options.theme.platform}`;
    //Remove theme that is duplicate with default theme
    themesArray = themesArray.filter((element) => {
      return element !== defaultTheme;
    });
    themesArray = themesArray.map((singleTheme) => {
      return _getValidThemeObject(singleTheme, platform);
    });
    return themesArray;
  } else {
    return undefined;
...
function _setSassCompile(theme, options) {
  ...
  if (theme.name === CONSTANTS.DEFAULT_THEME) {
    ...
    let srcExist = _checkPathExist(path.join(config("paths").src.common, config("paths").src.themes, theme.name));
    let themeExist = _checkPathExist(path.join(config("paths").staging.themes, theme.name));
    if (srcExist) {
