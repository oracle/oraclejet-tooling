...
function _runTypescriptCompilation(context) {
  return new Promise((resolve) => {
    ...
    // Figure out path to custom-tsc templates
    const pathToCustomTsc = path.join(util.getOraclejetPath(),
    ...
    if (fs.existsSync(componentsDtBaseSrcPath)) {
      ...
      if (files.length) {
        files.forEach((filepath) => {
          ...
          // Ensure that the generated (components_dt) component json
          // contains a valid pack property.
          if (util.hasProperty(componentJson, 'pack')) {
            // Here we have a valid pack property
            // in the generated (components_dt) component json
            componentJson.name = componentJson.name.replace(`${componentJson.pack}-`, '');
          } else if (componentsCache[componentJson.name] &&
            util.hasProperty(componentsCache[componentJson.name].componentJson, 'pack')
          ) {
            // Handle missing @metadata pack directive (from .tsx files).
            //   - Extract the pack name from componentsCache
            //   - Update the generated component json with the correct pack
            const cachePack = componentsCache[componentJson.name].componentJson.pack;
            componentJson.name = componentJson.name.replace(`${cachePack}-`, '');
            componentJson.pack = cachePack;
          }
          // Handle missing @metadata version directive (from .tsx files).
          // For missing version, copy from pack
          if (componentsCache[componentJson.pack] && util.hasMissingProperty(componentJson, 'version')) {
            componentJson.version = componentsCache[componentJson.pack].componentJson.version;
          }
          // Handle missing @metadata jetVersion directive (from .tsx files).
          // For missing jetVersion, copy from pack if defined in pack.
          // Otherwise, assign to util.getJETVersion().
          if (util.hasMissingProperty(componentJson, 'jetVersion')) {
            if (componentsCache[componentJson.pack] &&
                util.hasProperty(componentsCache[componentJson.pack].componentJson, 'jetVersion')) {
              componentJson.jetVersion =
                componentsCache[componentJson.pack].componentJson.jetVersion;
            } else {
          ...
          // vcomponent used to define the HTML of the app. Because its a custom element vcomponent,
          // the compiler will create a *.json file for it in components_dt
          // but we don't care about it and hence the util.isWebComponent check.
          if (util.isWebComponent({
